...
online retailer, Dec 2009 to Dec 2011) is analysed to identify entries warranting further
...
and unsupervised machine learning (Isolation Forest), combined into a single composite
risk score that ranks every scored transaction for review. A layered approach is
necessary because, as the results below show, no single method catches every class of
...
- **Online Retail II** (UCI Machine Learning Repository, dataset 502), real
  transaction-level data, {{cleaning_ledger.rows_out|comma}} cleaned rows, Dec 2009 to Dec
...
| Composite score, Average Precision | {{model_metrics.models.composite.average_precision|round4}} (95% CI {{model_metrics.models.composite.ap_ci95.0|round4}} to {{model_metrics.models.composite.ap_ci95.1|round4}}) |
| Precision at 500 reviewed | {{kpi_summary.headline_precision_at_500|pct1}}, reviewing the top 500 of {{kpi_summary.scored_population|comma}} scored transactions surfaces {{kpi_summary.headline_recall_at_500|pct1}} of the planted anomalies |
...
row-level review trigger. It fires on roughly 93% of every population studied, real or
...
> aggregate population also tested {{kpi_summary.aggregate_verdict}}. The finding holds at
...
Try the live dashboard first: **[vishalyl.github.io/benfords-audit-analytics](https://vishalyl.github.io/benfords-audit-analytics/)**.
It includes a live re-scoring simulator: pick a review budget and the browser recomputes
precision, recall and lift instantly from the real per-transaction ranking the pipeline
produced.

- **Static summary** (`docs/index.html`): dependency-free, dark-themed, always-live
  instant-load page generated by `src/static_dashboard.py`. No server, no OAuth step.
  This is what's live at the link above.
- **Streamlit app** (`app/streamlit_app.py`): the fuller interactive experience, 5 tabs,
  filters, segment selector, and drill-down, also dark-themed. Run locally with
  `streamlit run app/streamlit_app.py`, or deploy to Streamlit Community Cloud (one
  manual GitHub OAuth click at `share.streamlit.io`).
- **Power BI**: prepared-data-only this run. Power BI Desktop is a Windows GUI
  ...
  and the build steps are written out in full in `dashboard/POWERBI_BUILD_NOTES.md`.
...
The raw `online_retail_II.xlsx` is not committed (see `.gitignore`). Download it from the
UCI landing page and place it in `data/raw/` before running `src.ingest`.
...
checks/      one gate script per stage (gate_00.py .. gate_13.py)
data/        raw (git-ignored) / interim (git-ignored) / processed (git-ignored,
             except the injection log) / dashboard (committed, under 25MB)
...
A short, practical list, not a wishlist: **[`docs/roadmap_status.md`](docs/roadmap_status.md)**.
The full Phase 2 menu this was trimmed from is `plan/05_FUTURE_ROADMAP.md`.
