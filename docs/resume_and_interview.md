...
- Produced a 6-page audit workpaper (reportlab, regenerates from computed metrics) and
  ...
  achieving 40.2% precision in the top 500 reviewed items against a 1.6% random-
  selection baseline (a 25x lift).
...
- Computed Average Precision with 95% bootstrap confidence intervals (200 resamples)
  for every detection method — composite score AP 0.1817 [0.160, 0.211] vs. a 0.016
  random baseline, beating both Isolation Forest (0.136) and LOF (0.083) individually.
- Ran a 5-variant weight-sensitivity analysis on a judgement-set composite score
  ...
  disclosing that two alternative weightings scored marginally higher AP (0.191, 0.192
  vs. 0.182) instead of silently switching to the best-scoring variant.
...
composite risk score. The headline number: reviewing the top 500 of 50,000 scored
transactions surfaces about 25% of the planted anomalies, a 25x lift over random
selection. It's live at [repo link]."
...
(including `rule_flag_count`) on a 50,000-row sample rather than the full FS-A/FS-B
comparison — a scope limitation I've disclosed rather than hidden (see `DECISIONS.md`
D-0003), and the next concrete step to close.
...
Directly from the recall-vs-effort chart: reviewing the top 500 of 50,000 scored
transactions surfaces about 25% of known anomalies (a 25x lift over random); reviewing
...
Re-run Isolation Forest full-population with the FS-A/FS-B split and the contamination
sensitivity grid the design specifies, instead of the single 50,000-row sample this run
evaluated; recalibrate the Benford segment classification thresholds so the flag stops
over-triggering; and actually build the Power BI `.pbix` from the already-prepared data.
...
| Scored (evaluation) population | 50,000 rows |
| Isolation Forest AP | 0.136 (95% CI 0.119–0.157) |
| LOF AP | 0.083 (95% CI 0.066–0.104) |
| Composite AP | 0.182 (95% CI 0.160–0.211) |
| Random baseline precision | 0.016 |
| Precision @ 500 reviewed | 40.2% |
...
> 50,000 ranked transactions surfaces ~25% of the planted anomalies, a 25x lift over
> random selection.
>
